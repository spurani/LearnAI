# -*- coding: utf-8 -*-
"""
Created on Sat Sep 18 19:28:31 2021
...
def process(image_original):
    ...
    f22 = cv2.imread("f22raptor.jpg")
    f22raptor = cv2.resize(f22,(mask.shape[1],mask.shape[0]))
    img_new = cv2.bitwise_or(f22raptor,mask)
    cv2.imshow("combo",img_new)
    cv2.imshow("mask",mask)
    image_hsv[:,:] = np.bitwise_and(image_hsv[:,:], img_new)
